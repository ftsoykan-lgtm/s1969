...
export default function AdminSidebar({ logoUrl }: { logoUrl?: string | null }) {
  ...
  return (
    <aside className="w-56 shrink-0 bg-ugreend min-h-screen flex flex-col">
      {/* Logo */}
                ...
                return (
                  <Link key={item.href} href={item.href}
                    className={cn(
                      ...
                      active ? 'bg-ugold text-ugreend' : 'text-white/55 hover:text-white hover:bg-white/8'
                    )}>
                    <item.icon size={14} className={active ? 'text-ugreend' : 'opacity-60 group-hover:opacity-100'} />

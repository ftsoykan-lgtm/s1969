                  ...
                  return (
                    <div key={id + i} className="bg-white rounded-2xl border border-[#ddeae2] shadow-sm overflow-hidden">
                      <button onClick={() => setOpen(isOpen ? null : id)}
                        ...
                        {r._orig && <Link href={`/sayfa/${r.slug}`} target="_blank" onClick={(e) => e.stopPropagation()} className="text-[#7aab8e] hover:text-[#1A6B3C] p-1"><ExternalLink size={14} /></Link>}
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto_auto] gap-3 items-end">
                            <Field label="Kapak Görseli (opsiyonel)">
                              <div className="flex items-center gap-1.5">
                                ...
                                <label className="shrink-0 cursor-pointer p-2 text-[#7aab8e] hover:text-[#1A6B3C] hover:bg-[#edf7f2] rounded-lg transition-all">
                                  <Upload size={15} />
                                  <input type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) handleHero(i, f) }} />
                                </label>
                              </div>
                            </Field>
                            <label className="flex items-center gap-2 text-sm font-bold text-[#3d6b52] pb-2">
                              <input type="checkbox" checked={r.published ?? true} onChange={(e) => update(i, { published: e.target.checked })} className="w-4 h-4 accent-[#1A6B3C]" />
                              Yayında
                            </label>
                          </div>
...
function Banner({ tone, children }: { tone: 'err' | 'ok'; children: React.ReactNode }) {
  const cls = tone === 'err' ? 'bg-red-50 border-red-200 text-red-700' : 'bg-[#edf7f2] border-[#1A6B3C]/20 text-[#1A6B3C]'

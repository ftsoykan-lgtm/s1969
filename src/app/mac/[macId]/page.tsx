...
function EventIcon({ type }: { type: MatchEvent['type'] }) {
  ...
  if (type === 'yellow') return <span className="inline-block w-3 rounded-[2px] bg-ugold shadow" style={{ height: 17 }} />
  if (type === 'red') return <span className="inline-block w-3 rounded-[2px] bg-[#d01b2a] shadow" style={{ height: 17 }} />
  return <span className="text-ugreen">↔</span>
}
      ...
      <div className="relative bg-gradient-to-b from-[#0c3a23] to-ugreenm overflow-hidden">
        ...
        <div className="relative mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-10">
          <Link href="/mac-merkezi"
            className="group inline-flex items-center gap-2 mb-8 rounded-full bg-white/[0.07] hover:bg-ugold border border-white/15 hover:border-ugold pl-2.5 pr-4 py-2 transition-all">
        ...
        {hasLineups ? (
          ...
          <Card title="İlk 11">
            <div className="py-8 text-center">
              <p className="text-sm font-bold text-ugreenm">Kadrolar henüz açıklanmadı</p>
              ...
              {referees.map((r, i) => {
                ...
                return (
                  <div key={r.name + r.role}
                    className={`flex items-center gap-3 rounded-xl px-4 py-3.5 border ${main ? 'bg-gradient-to-br from-ugreend to-[#0c3a20] border-transparent' : 'bg-[#f8faf9] border-[#edf7f2]'}`}>
                    <span className={`flex h-9 w-9 items-center justify-center rounded-lg shrink-0 ${main ? 'bg-ugold text-ugreend' : 'bg-[#edf7f2] text-ugreen'}`}>
                      <Flag size={15} />
                    </span>
...
function Chip({ icon: Icon, children }: { icon: React.ComponentType<{ size?: number; className?: string }>; children: React.ReactNode }) {
  return (
    <span className="inline-flex items-center gap-1.5 bg-white/[0.07] border border-white/10 rounded-full px-3.5 py-1.5 text-xs font-semibold text-white/70">
      <Icon size={13} className="text-ugold" />{children}
    </span>
  )
}
...
function Milestone({ emoji, anim, label, score, tone }: { emoji: string; anim?: string; label: string; score?: string; tone: 'green' | 'amber' }) {
  const bg = tone === 'amber' ? 'bg-ugold text-ugreend' : 'bg-ugreend text-white'
  return (
...
function ElevenList({ team, logo, players, coach, variant = 'opp', muted = false, profileSlugs }: { team: string; logo: string; players: LineupPlayer[]; coach?: string | null; variant?: 'sfk' | 'opp'; muted?: boolean; profileSlugs?: Set<string> }) {
  ...
  const badge = muted
    ? (isSfk ? 'bg-[#e3f1e9] text-ugreen' : 'bg-[#eef1f4] text-[#475569]')
    : (isSfk ? 'bg-gradient-to-br from-ugreen to-ugreend text-ugold' : 'bg-gradient-to-br from-[#475569] to-[#334155] text-white')
  const stripe = isSfk ? 'bg-ugold' : 'bg-[#475569]'
  return (
    <div className="relative rounded-xl border border-[#edf7f2] overflow-hidden">
      ...
      {coach && <p className="px-4 pb-3 text-xs text-[#7aab8e]">Teknik Direktör: <span className="font-bold text-ugreenm">{coach}</span></p>}
    </div>
  )
}
...
function Info({ icon: Icon, label, value }: { icon: React.ComponentType<{ size?: number; className?: string }>; label: string; value: string }) {
  return (
    <div className="flex items-center gap-3.5 bg-gradient-to-br from-[#f8faf9] to-[#eef5f0] rounded-xl px-4 py-4 border border-[#edf7f2]">
      <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br from-ugreen to-ugreend text-ugold shrink-0 shadow-md">
        <Icon size={18} />
      </div>
      <div className="min-w-0">

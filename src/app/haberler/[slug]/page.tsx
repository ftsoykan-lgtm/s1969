  ...
  return (
    <div className="min-h-screen bg-[#f5f9f6]">
      ...
      <div className="relative h-[300px] md:h-[440px]">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#154836]/98 via-[#103f2e]/60 to-transparent" />
      </div>
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 -mt-28 relative pb-16">
        ...
        <article className="bg-white rounded-2xl border border-[#ddeae2] shadow-lg overflow-hidden">
          <div className="p-6 md:p-10">
            <span className="inline-flex items-center px-3 py-1 rounded-full text-[11px] font-black tracking-widest uppercase mb-5 bg-[#f5c400] text-[#154836]">
              {catName}
            </span>
            <h1 className="text-3xl md:text-4xl font-black text-[#154836] leading-tight tracking-tight mb-4">{item.title}</h1>
            <div className="flex items-center gap-2 text-sm text-[#356152] mb-8 pb-8 border-b border-[#ddeae2]">
              <Calendar size={14} />
              <span>{formatDate(item.date)}</span>
            </div>
            {item.excerpt && <p className="text-lg text-[#356152] font-medium leading-relaxed mb-6">{item.excerpt}</p>}
            <div className="text-[#154836] leading-relaxed text-base whitespace-pre-line">{item.content}</div>
          </div>
        </article>

        {related.length > 0 && (
          <div className="mt-12">
            <h2 className="text-xl font-black text-[#154836] mb-5">İlgili Haberler</h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {related.map((rel) => (
                <Link key={rel.id} href={`/haberler/${rel.slug}`}
                  className="group relative block rounded-xl overflow-hidden h-48 border border-[#ddeae2] shadow-sm hover:shadow-md hover:border-[#1b5e44]/40 transition-all">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-[#154836]/95 to-transparent" />
                  <div className="absolute bottom-0 left-0 right-0 p-4">
                    <p className="text-white text-sm font-bold line-clamp-2">{rel.title}</p>
                    <p className="text-[11px] text-[#f5c400]/70 mt-1">{formatDate(rel.date)}</p>
                  </div>
                </Link>
              ))}
            </div>

      ...
      <div className="bg-[#1b5e44] py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 mb-3">
            ...
            <p className="text-xs font-black tracking-widest uppercase text-[#f5c400]/60">Maç Biletleri</p>
          </div>
          <h1 className="text-4xl md:text-5xl font-black text-white tracking-tight">
            Bilet <span className="text-[#f5c400]">Al</span>
          </h1>
        </div>
      </div>

      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-12 pb-20 space-y-6">
        <div className="flex items-start gap-3 bg-[#f5c400]/10 border border-[#f5c400]/30 rounded-xl p-4">
          <Info size={16} className="text-[#c9a200] mt-0.5 shrink-0" />
          <p className="text-sm text-[#154836]">Biletler çevrimiçi satın alınabilir veya GAP Arena gişesinden temin edilebilir. Üye indirimi için giriş yapın.</p>
        </div>

        {matches.map((m) => (
          <div key={m.id} className="bg-white rounded-2xl border border-[#ddeae2] shadow-sm overflow-hidden">
            <div className="bg-[#103f2e] px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <p className="text-[#f5c400] text-xs font-black tracking-widest uppercase mb-1">Ev Sahibi Maçı</p>
            ...
            <div className="p-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                { tier: 'Uçurtma Tribün', price: m.prices.ucurtma, color: 'border-[#ddeae2]', badge: 'bg-[#edf7f2] text-[#1b5e44]' },
                { tier: 'Orta Tribün', price: m.prices.orta, color: 'border-[#1b5e44]/40', badge: 'bg-[#1b5e44] text-white' },
                { tier: 'VIP Tribün', price: m.prices.vip, color: 'border-[#f5c400]/60', badge: 'bg-[#f5c400] text-[#154836]' },
                ...
                <div key={tier} className={`rounded-xl border-2 ${color} p-4 flex flex-col gap-3`}>
                  ...
                  <p className="text-2xl font-black text-[#154836]">₺{price}</p>
                  <button className="w-full bg-[#1b5e44] hover:bg-[#103f2e] text-white text-sm font-black py-2.5 rounded-xl transition-colors flex items-center justify-center gap-2">

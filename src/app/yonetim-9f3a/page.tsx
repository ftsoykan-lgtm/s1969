...
export default async function AdminDashboard() {
  ...
  const stats = [
    { label: 'Haberler', value: fmt(haberler), icon: Newspaper, href: '/yonetim-9f3a/haberler', color: 'bg-[#edf7f2] text-ugreen' },
    { label: 'Oyuncular', value: fmt(oyuncular), icon: Users, href: '/yonetim-9f3a/kadro', color: 'bg-ugold/15 text-ugoldd' },
    { label: 'Sayfalar', value: fmt(sayfalar), icon: FileText, href: '/yonetim-9f3a/sayfalar', color: 'bg-[#edf7f2] text-ugreen' },
    { label: 'Sponsorlar', value: fmt(sponsorlar), icon: Star, href: '/yonetim-9f3a/sponsorlar', color: 'bg-ugold/15 text-ugoldd' },
  ]

  const sections = [
    { label: 'Haberler', desc: 'Haber ekle, düzenle, story', href: '/yonetim-9f3a/haberler', icon: Newspaper },
    { label: 'Kategoriler', desc: 'Haber kategorileri', href: '/yonetim-9f3a/kategoriler', icon: FolderOpen },
    { label: 'Sayfalar', desc: 'Kulüp, tesisler, kurumsal sayfalar', href: '/yonetim-9f3a/sayfalar', icon: FileText },
    { label: 'Kadro & Oyuncular', desc: 'Oyuncu profilleri, sezon', href: '/yonetim-9f3a/kadro', icon: Users },
    { label: 'Sponsorlar', desc: 'Ana / resmi / destekçi', href: '/yonetim-9f3a/sponsorlar', icon: Star },
    { label: 'Takım Logoları', desc: 'Rakip + turnuva logoları', href: '/yonetim-9f3a/logolar', icon: ImageIcon },
    { label: 'Site Ayarları', desc: 'Kulüp bilgileri, SEO, video', href: '/yonetim-9f3a/ayarlar', icon: Settings },
  ]
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap gap-3">
        <Link href="/yonetim-9f3a/haberler" className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-extrabold bg-ugreen text-white hover:bg-ugreend transition-colors">Yeni Haber Ekle <ArrowUpRight size={13} /></Link>
        <Link href="/yonetim-9f3a/kadro" className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-extrabold bg-ugold text-ugreenm hover:brightness-105 transition-all">Kadroyu Güncelle <ArrowUpRight size={13} /></Link>
        <Link href="/yonetim-9f3a/sayfalar" className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-extrabold border border-[#ddeae2] text-ugreenm hover:bg-[#f5f9f6] transition-colors">Sayfa Düzenle <ArrowUpRight size={13} /></Link>
      </div>
